...
def train(args, model):
    ...
    if args.log_txt:
        sys.stdout = open('./' + args.tag + '_' + args.model_type.lower() + '_' + str(args.stem_width) + '_' + args.distance_type + '_trainer_output.txt', 'a')
    
    directory = os.path.dirname(args.tag + '_' + args.model_type.lower() + '_' + str(args.stem_width) + '_' + args.distance_type + '_checkpoint/')
    ...
    if (args.val_image_dir is None) or (args.val_img_uid_map is None):
        ...
        print('Train Dataset Length:', len(train_dataset), ', Val Dataset Length:', len(val_dataset), flush=True)
        ...
        for batch, data in enumerate(loader):
            ...
            if args.cuda:
                ...
                with torch.autocast(device_type='cuda', dtype=torch.float16):
                    ...
                    loss = batch_hard_triplet_loss(labels, outputs, margin=args.margin, distance_type=args.distance_type)
            ...
            if batch % args.log_batch== 0:
                ...
                print("Train loss: {aver} (epoch: {epoch}, batch: {batch}/{total})".format(aver = train_loss_average, epoch = epoch, batch = batch, total=len(loader)), flush=True)
        ...
        if len(val_loader) > 0:
            ...
            with torch.inference_mode():
                for batch, data in enumerate(val_loader):
                    ...
                    val_loss, _ = batch_all_triplet_loss(labels, outputs, margin=args.margin, distance_type=args.distance_type)
                    
                    val_epoch_loss.append(val_loss.item())
                
                val_loss_average = sum(val_epoch_loss) / len(val_epoch_loss)
                print("Val loss: {aver} (epoch: {epoch})".format(aver = val_loss_average, epoch = epoch), flush=True)
                if args.log_txt:
                    sys.stdout.close()
                    sys.stdout = open('./' + args.tag + '_' + args.model_type.lower() + '_' + str(args.stem_width) + '_' + args.distance_type + '_trainer_output.txt', 'a')
                if val_loss_average < best_val_loss_average:
                    ...
                    filename = os.path.join(directory, "{model}-{epoch:03}-{val}.pth".format(model = args.model_type, epoch = epoch, val=round(val_loss_average, 6)))
                    if args.multi_gpu:
...
def main(args):
    ...
    if model_type.startswith('convnext'):
        lastconv_output_channels = {'convnext_tiny': 768, 'convnext_small': 768, 'convnext_base': 1024, 'convnext_large': 1536}
